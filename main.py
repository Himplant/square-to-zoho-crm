...
async def _handle_square(req: Request):
    ...
    booking_id = payload.get("data", {}).get("id") or payload.get("data", {}).get("object", {}).get("id")
    if not booking_id:
        # Some Square test payloads wrap deeper; but we'll fail fast
        raise HTTPException(400, "Missing booking id")
    ...
    if not (start_at and end_at):
        raise HTTPException(400, "Booking missing start or end")

...
def normalize_phone(phone: Optional[str]) -> str:
    ...
    if digits and not digits.startswith("+"): digits = "+" + digits
    return digits
...
def zoho_find_person(token: str, email: Optional[str], phone: Optional[str]):
    email = (email or "").strip()
    phone = normalize_phone(phone)
    if email:
        r = zoho_search_module(token, "Contacts", f"(Email:equals:{email})")
        if r: return "Contacts", r
    if phone:
        for f in ("Phone","Mobile"):
            r = zoho_search_module(token, "Contacts", f"({f}:equals:{phone})")
            if r: return "Contacts", r
    if email:
        r = zoho_search_module(token, "Leads", f"(Email:equals:{email})")
        if r: return "Leads", r
    if phone:
        for f in ("Phone","Mobile"):
            r = zoho_search_module(token, "Leads", f"({f}:equals:{phone})")
            if r: return "Leads", r
    return None, None

def zoho_create_lead(token: str, first_name: str, last_name: str, email: str, phone: str) -> str:
    url = f"{ZOHO_CRM_BASE}/crm/v2/Leads"
    payload = {"data":[{"First_Name": first_name or "","Last_Name": last_name or (email or phone or "Unknown"),
...
def zoho_upsert_event(token: str, who_id: str, subject: str, start_iso: str, end_iso: str, desc: str=""):
    ...
    payload = {"data":[{"Subject":subject,"Start_DateTime":start_iso,"End_DateTime":end_iso,
                        "Description":desc,"Who_Id":{"id":who_id}}]}
    r = requests.put(url, headers=Z(token), json={"data":[{**payload["data"][0], "id": existing["id"]}]}, timeout=30) if existing \
        else requests.post(url, headers=Z(token), json=payload, timeout=30)
    if r.status_code not in (200,201,202):
...
def zoho_convert_lead(token: str, lead_id: str, deal_name: str):
    ...
    payload = {"data":[{"overwrite":True,"notify_lead_owner":False,"notify_new_entity_owner":False,
                        "Deals":{"Deal_Name":deal_name,"Pipeline":DEFAULT_PIPELINE,"Stage":DEFAULT_DEAL_STAGE}}]}
    r = requests.post(url, headers=Z(token), json=payload, timeout=30)
    if r.status_code not in (200,201,202):
...
def zoho_create_deal_for_contact(token: str, contact_id: str, deal_name: str):
    ...
    payload = {"data":[{"Deal_Name":deal_name,"Pipeline":DEFAULT_PIPELINE,"Stage":DEFAULT_DEAL_STAGE,
...
def zoho_mark_event_canceled(token: str, subject: str):
    ev = zoho_search_module(token, "Events", f"(Subject:equals:{subject})");  if not ev: return
    url = f"{ZOHO_CRM_BASE}/crm/v2/Events"
    ...
    requests.put(url, headers=Z(token), json=payload, timeout=30)
...
def zoho_update_deal_stage(token: str, deal_id: str, stage: str):
    url = f"{ZOHO_CRM_BASE}/crm/v2/Deals"
    requests.put(url, headers=Z(token), json={"data":[{"id":deal_id,"Stage":stage}]}, timeout=30)

def sq_get_booking(booking_id: str) -> dict:
    r = requests.get(f"https://connect.squareup.com/v2/bookings/{booking_id}",
                     headers={"Authorization": f"Bearer {SQUARE_ACCESS_TOKEN}"}, timeout=30)
    if r.status_code != 200:
...
def sq_get_customer(customer_id: str) -> dict:
    r = requests.get(f"https://connect.squareup.com/v2/customers/{customer_id}",
                     headers={"Authorization": f"Bearer {SQUARE_ACCESS_TOKEN}"}, timeout=30)
    if r.status_code != 200:
